...
#%%
indexed = [
    pd.Series(series.values[series.values.nonzero()[0]], name=series.name)
...
# sns.relplot(x=eggerino.index, y="" data=eggerino)

#%% Transform into a more plottable shape


def get_rehaped_data(series):
    data_for_frame = {}
    # Get the values which are not zero
    data = series.values
    data_for_frame["value"] = data[data.nonzero()[0]]
    # Set a time axis in range of nonzero data
    data_for_frame["time"] = np.arange(len(data_for_frame["value"])) + 1
    # Name as all data frame indices
    indexum = np.tile(series.name, len(data_for_frame["value"]))
    return pd.DataFrame(data_for_frame, index=indexum)
...
a = pd.concat(
    [get_rehaped_data(eggerino.iloc[idx]) for idx in range(eggerino.shape[0])]
)

#%%

ax = sns.relplot(x="time", y="value", hue=a.index, kind="line", data=a)
ax.set(
    xlabel="Number of unique transitions",
    ylabel="Average IRT at unique tranistion",
    title="Average IRT per Transition to New Memory",
)

# #%%
# sns.lineplot(data=a)

# sns.lineplot(x="time", y="value", data=a)
#%%


#%%
counts_distribution = pd.DataFrame()
for idx in range(len(eggerino.shape[0])):
    if idx == 0:
        continue
    counts_distribution = counts_distribution.append(
        recalls_analysis_data_frames_all[idx]
...
for num_unique_recalls in list(eggerino.index):
    data = eggerino.loc[num_unique_recalls].values
    # data = data[data.nonzero()[0]]
    datu = pd.DataFrame
    ...
    # print(data)
    ax = sns.lineplot(data=data, legend=data.index)
